    ...
    return (
      <Card onPress={onPress} className="p-4">
        <View className="flex-row items-start justify-between mb-2">
          ...
          {post.expiry_date && (
            <Text className="text-sm font-nunito text-brand-ink">Expires {post.expiry_date}</Text>
          )}
        </View>
  ...
  return (
    <Card onPress={onPress} className="overflow-hidden">
      ...
      <View className="p-4">
        ...
        <Text className="text-base font-nunito-semibold text-brand-ink mb-1 leading-relaxed">{promo.headline}</Text>
        {promo.description && (
          <Text className="text-base font-nunito text-brand-ink leading-relaxed mb-3" numberOfLines={3}>
            {promo.description}
          </Text>
        )}
