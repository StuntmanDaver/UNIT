import type { AdminPromotion, AdminPromotionReviewAction } from './types';

export type PromotionWorkflowSnapshot = Pick<AdminPromotion, 'review_status' | 'payment_status'>;

export type PromotionReviewDecision = {
  update: Partial<Pick<AdminPromotion, 'review_status' | 'payment_status' | 'reviewed_by' | 'reviewed_at' | 'review_note'>>;
  event: {
    from_review_status: AdminPromotion['review_status'];
    to_review_status: AdminPromotion['review_status'];
    from_payment_status: AdminPromotion['payment_status'];
    to_payment_status: AdminPromotion['payment_status'];
...
export type PromotionSuspensionDecision = {
  update: Partial<Pick<AdminPromotion, 'review_status' | 'reviewed_by' | 'reviewed_at'>>;
  event: {
...
export function canReviewPromotion(snapshot: Pick<AdminPromotion, 'review_status'>): boolean {
  return snapshot.review_status === 'pending';
}

export function canSuspendPromotion(snapshot: Pick<AdminPromotion, 'review_status'>): boolean {
...
export function deriveReviewDecision(
  ...
  let paymentStatus: AdminPromotion['payment_status'] = snapshot.payment_status;
...
export function deriveSuspensionDecision(
  snapshot: Pick<AdminPromotion, 'review_status'> & { review_status: 'approved' | 'suspended' },
  actorUserId: string,
  nowIso: string
