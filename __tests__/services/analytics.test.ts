...
describe('analyticsService.recordTap', () => {
  beforeEach(() => jest.clearAllMocks());

  it('inserts a tap event', async () => {
    const { supabase } = require('@/services/supabase');
    await analyticsService.recordTap('promo-1', 'tenant-1', 'prop-1', 'sess-abc');
    expect(supabase.from).toHaveBeenCalledWith('ad_analytics');
    expect(mockInsert).toHaveBeenCalledWith({
      promotion_id: 'promo-1',
      tenant_id: 'tenant-1',
  ...
  it('throws on database errors (no silent swallow unlike recordView)', async () => {
    mockInsert.mockResolvedValueOnce({ error: { code: '42P01', message: 'table missing' } });
    await expect(
      analyticsService.recordTap('promo-1', 'tenant-1', 'prop-1', 'sess-abc')
    ).rejects.toThrow();
  });
});
